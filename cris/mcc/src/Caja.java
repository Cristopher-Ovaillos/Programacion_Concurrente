...
public class Caja {
    ...
    private Condition camion, seguirllenando;
    ...
    private int almacen, capacidadAlmacen = 2;
    // tipo
    private String tipo;
    ...
    public Caja() {
        ...
        this.seguirllenando = mutex.newCondition();
    }
    ...
    public void empaquetar() throws InterruptedException {
        ...
        while (true) {
            ...
            // puede seguir llenando??? me refiero a que si el camion esta lleno el
            // aviso al camion que ya puse una caja en el almacen
            camion.signal();
            // esta lleno entonces tengo que evitar que el empaquetador siga agregando cajas
            // al almacen 1*
            seguirllenando.await();
            System.out.println("");
            mutex.unlock();
        }
    ...
    // metodo usado por el main
    public void almacenLleno() throws InterruptedException {

        while (true) {

            mutex.lock();
            // va estar en espera hasta que este lleno el almacen
            while (capacidadAlmacen != almacen) {
                // si no esta lleno estonces el empaquetador puede seguir llenando el almacen 2*
                seguirllenando.signal();
                camion.await();

            }
            System.out.println("");
            System.out.println("Viene el camion.");
            System.out.println("Se lleva las " + capacidadAlmacen + " cajas.");
            System.out.println("Queda vacio el almacen.");
            almacen = 0;
            System.out.println("");
            // ya se vacio entonce puede seguir llenando
            seguirllenando.signal();
